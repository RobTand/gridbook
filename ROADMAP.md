...
| **Naive CUDA-graph capture of the decode path** | Measured *worse*, not better: vLLM pads captured decode batches past the prefill threshold, so every graphed step took the prefill path. `--enforce-eager` is the tested configuration; capture is supported only on the host-branch-free paths. |
