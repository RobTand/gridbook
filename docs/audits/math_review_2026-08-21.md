...
Status: COMPLETE (byte-law audit, lemma verifications, new proofs, kernel landings B1/B2/B7)
...
Method: independent numeric verification via scratch scripts outside the repo (`/tmp/opencode/matheval/`, reports `/tmp/opencode/reports/`), first-principles derivations with named numerical anchors, and the repo's own bit-exactness suites run in `gridbook:0.8.11-clean-187c721` on the GB10.
...
- Spill predicate exactness: enumeration over all 21 rungs × 32 codewords matches the predicate bit-exactly (5376 cases); closed forms locate the only live shipped cell: `k > 32 + gcd(k,32)` (u32 windows) / `k > 32 + gcd(k,8)` (u64 bursts) ⇒ persistent-B k=44 is the sole rung whose hot loop must read the third word — which is precisely why the grouped kernel's predicated load is bit-neutral and profitable elsewhere.
...
## §5 Staging vectorization precondition theorem (feeds landed B2)

Theorem (staging byte-neutrality): uchar4/u32 vectorized copies are byte-identical to the byte-granular loop for type_size ≡ 0 (mod 4), and for odd type_size = 4k+9 provided (i) misaligned source phases are absorbed by align-down + funnel-shift on the LOAD side while dst positions stay absolute, and (ii) the seam zero-fill of [type_size, ts_pad) is preserved — negative test shows drift appears exactly when ts mod 4 ≠ 0 without the seam discipline. Landed implementation B2 satisfies all five stated preconditions (P1–P5 in-source); decode-probe torch.equal gates green (150 passed / 28 skipped).
...
### B2 — persistent-B packed-staging vectorization (landed)

Byte-granular staging → u32 interior with funnel-shift edges, whole-slot zeroing helper; `__noinline__` scoping protects mainloop register allocation (inlining measured +26 registers/thread ⇒ ~23% whole-operator regression — the guard is load-bearing). Bit-neutrality per §5; suites green. Follow-up measurement cell (staging-share isolation) may be attached to the next persistent-B A/B run.
...
| G7 | Byte-granular staging instruction waste | PERF | FIXED by B2 |

Verification artifacts: in-repo: `docs/audits/math_review_2026-08-21_proofs.md` (full proofs incl. ρ-tightness Thms 4–7, window lemma, staging theorem); working reports under `/tmp/opencode/reports/`.
