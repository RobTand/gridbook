...
| `pq_cb_fused` (FP8-CB mid-M fused CUTLASS prefill) | `gridbook/csrc/cb_fused_gemm.cu` + `csrc/cutlass_fork/*.hpp` | **at weight load** when the loading device is cc 12.0/12.1 and `PRISMAQUANT_CB_FUSED_MIDM` is auto/unset or `1`; availability and mode are fixed before serving, and later environment mutation raises. Auto on SM89 never queries this loader; explicit `1` there fails the load. | **Optional Blackwell specialization under auto.** An unavailable auto specialization stays on the exact native FP8 expansion/CUTLASS route; explicit `1` instead fails the model load. It is never first-built inside an eligible forward. |
