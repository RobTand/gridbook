...
| **Memory** | 23.0 GB weights + KV cache. Reference box: one 128 GB unified-memory GB10 at `--gpu-memory-utilization 0.90`. A 32 GB consumer Blackwell (RTX 5090, `sm_120`) is the intended target but is **not currently verified**: an external report hits OOM at 30.5 GiB — [issue #1](https://github.com/RobTand/gridbook/issues/1), open. |
| **vLLM** | Not recorded for this card's measurements. This artifact has been exercised on vLLM 0.25.1 and 0.23.1rc1.dev1060 by an external reporter ([issue #1](https://github.com/RobTand/gridbook/issues/1)). |
