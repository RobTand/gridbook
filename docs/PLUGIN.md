...
- **Routed FP8-CB whole-row sibling** `cb_moe_gemv_fp8_v2`: an independent,
  default-off alternative selected only by
  `PRISMAQUANT_CB_FP8_GEMV_V2=1`. It stages one complete packed weight row and
...
| `PRISMAQUANT_CB_FP8_GEMV_V2` | off (`0`) | `1` selects the routed FP8-CB whole-row sibling for exactly `k=28/n_sub=4/type_size=112` at K=2048/4096; unset or literal `0` keeps the inherited grouped FP8 kernel. Only literal `0` and `1` are accepted, and the value is process-stable. The independently selected FP4 route and source-passthrough groups are unchanged. An explicit candidate fails at model load for an unsupported uniform FP8 cell or per-expert mixed FP8-CB group, and the main extension's strict ABI rejects a stale `.so` without `cb_moe_gemv_fp8_v2` even when this selector is off. Exact eager quality is zero-delta, but the prior ~7.2% served cycle signal is held because it used a different binary and failed content/acceptance integrity; final-binary served graph, throughput, concurrency, soak, long-prefill, and memory gates remain open. |
