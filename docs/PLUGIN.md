...
| `PRISMAQUANT_CB_GEMV` | `auto` | Which grouped fp4-CB decode GEMV serves a layer: `auto` \| `v2` \| `inherited`. `auto` uses the smem-resident-dictionary kernel (`cb_gemv_v2.cu`) only where a compiled occupancy predicate says it wins, and never builds it for an fp8-CB-only serve. `inherited` is the kill switch and the A/B control — it reproduces the shipped dispatch exactly and never attempts the v2 JIT build. **Not** bit-exact against `inherited` (reassociation class). An unknown spelling raises; changing it mid-process raises. |
