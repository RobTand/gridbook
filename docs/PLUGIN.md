...
- `cb_fused_fp4_moe_grouped` is the qualified grouped arithmetic primitive with
  explicit TileM128/256 runners. Dense `static_lsq` is not yet wired into the
  MoE serving selector. Kernel parity and raw speed do not promote either flag;
...
| `PRISMAQUANT_CB_FUSED_FP4_MOE` | off | Grouped-MoE fp4-CB native-FP4 prefill opt-in. Static `1`/`128` and `256` select TileM 128 and 256 and require both attested stage scalars. `rowwise`/`rowwise128` and `rowwise256` select the same tiles with independent runtime row scales and may serve legacy artifacts. Dense `static_lsq` is not wired to this selector and supplies no MoE qualification. An ineligible attempt fails closed to `loop`; unknown spellings and mid-process changes fail. Keep this off pending the dated audit's routed-quality, workload, and routing-policy gates. |
