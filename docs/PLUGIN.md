...
| `PRISMAQUANT_CB_FUSED_FP4` | off | Dense fp4-CB native-FP4 prefill opt-in. `1`/`midm` use a fully attested artifact scalar for all prefill shapes / 16 < M <= 128. `rowwise`/`rowwise_midm` instead derive an independent full-range scalar per runtime row, serialize no new metadata, and are the only fused choices accepted for legacy artifacts. All values remain experimental and default-off; unknown spellings and mid-process changes fail. Keep it off pending served quality/performance gates; see the [dated audit](audits/fused_nvfp4_enablement_2026-07-31.md). |
| `PRISMAQUANT_CB_FUSED_FP4_MOE` | off | Grouped-MoE fp4-CB native-FP4 prefill opt-in. Static `1`/`128` and `256` select TileM 128 and 256 and require both attested stage scalars. `rowwise`/`rowwise128` and `rowwise256` select the same tiles with independent runtime row scales and may serve legacy artifacts. An ineligible attempt fails closed to `loop`; unknown spellings and mid-process changes fail. The new activation modes are not served promotion evidence; keep this off pending the dated audit's gates. |
