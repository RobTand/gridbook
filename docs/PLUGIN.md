...
| `PRISMAQUANT_CB_PREFILL` | `auto` (fp8-CB) / `stock` (fp4-CB) | MoE prefill path: `auto` \| `stock` \| `grouped_fused` \| `loop`. `auto` measures the candidates per layer on the first prefill and caches the choice; it is fp8-only in practice because every candidate besides `stock` is gated on fp8-CB, which is why fp4-CB goes straight to `stock`. `l2_pipeline` exists but is **diagnostic-only** — it wedged live serving three times. |
...
| `PRISMAQUANT_CB_PREFILL_CHUNK_BYTES` | `1073741824` (1 GiB) | Byte budget the **fp4-CB** stock chunk is sized against (largest chunk whose one-stage bf16 weight tile fits). fp4's transient is 2 B/elt with no CUDA expander, so a flat chunk of 256 holds every expert of a large MoE live — 4,736 MiB measured on the 192-expert Hy3 CB band vs 1,184 MiB at this budget. fp8-CB ignores it. |
