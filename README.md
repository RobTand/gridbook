...
| **Parallelism** | **Dense CB Linears shard above one rank; everything else is `tp=1`.** Dense CB weights load shard-aware at `--tensor-parallel-size > 1` (no exported-byte change; illegal group boundaries are refused at construction). MoE, delegated groups and passthrough units still refuse above one. No cross-node speedup is claimed or measured. |
