...
| **Parallelism** | **Dense CB Linears and dense FP8 source-passthrough Linears shard above one rank; everything else is `tp=1`.** Both load shard-aware at `--tensor-parallel-size > 1` with no exported-byte change; illegal boundaries are refused at construction (CB: whole packed groups; FP8 passthrough: whole 128-element source blocks per rank, per fused role). Grouped-BMM passthrough units shard only at a measured degree (1, 2, 4 at the DSv4 `wo_a` geometry, because sharding a grouped plane divides the kernel's group count). A mixed-format fused projection shards when every one of its roles does — the composer builds each carrier at that role's whole-tensor output size, so each role's own law decides. MoE, delegated groups and quantized embeddings still refuse above one. No cross-node speedup is claimed or measured. |
